...
const deleteCarrito = (req, res) => {
	...
	if (carritoDeleted === -1) return res.status(404).json({ message: 'El ID no pertenece a un carrito listado' });
...
const getProducts = (req, res) => {
	...
	if (carritoSelected == null) return res.status(404).send({ message: 'Ingresa el ID de un carrito listado' });
	res.json({ 'Productos': carritoSelected.products });
}

                                    /////////////  Agregar un producto al Carrito///////////////////

const addProductToCarrito = (req, res) => {
	const idCarrito = Number(req.params.id);
	if (isNaN(idCarrito)) return res.status(400).send({ message: 'Ingresa el ID de un carrito listado' });
	const { idProduct } = req.body;
	const productSaved = carrito.saveProduct(idCarrito, idProduct);
...
export { addCarrito, deleteCarrito, getProducts, addProductToCarrito, deleteProduct };